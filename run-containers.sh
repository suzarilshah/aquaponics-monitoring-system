#!/bin/bash
# Script to run the Aquaponics Monitoring System containers from GitHub Container Registry
...
echo -e "${YELLOW}Setting up Aquaponics Monitoring System containers${NC}"
echo -e "${YELLOW}=============================================================${NC}"
...
docker run -d \
  ...
  ghcr.io/suzarilshah/aquaponics-monitoring-system-server:latest
...
docker run -d \
  ...
  --health-retries=3 \
  --health-start-period=40s \
  ghcr.io/suzarilshah/aquaponics-monitoring-system-client:latest
